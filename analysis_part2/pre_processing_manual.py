...
subj= 'sub_27' #set to id/subject whose data you want to preprocess
...
epochs = mne.Epochs(raw, events, tmin=cfg["epochs"]["tmin"], tmax=cfg["epochs"]["tmax"],
                    ...
                    baseline=cfg["epochs"]["baseline"],
                    detrend=cfg["epochs"]["detrend"])  # Epoch data and apply baseline
...
bads=["AF8", "TP9", "P7"] #Add channel names to exclude here
if len(bads) != 0:
    for bad in bads:
...
ica.exclude = [0,1,2,5, 7,9] #List of integer components to exclude
